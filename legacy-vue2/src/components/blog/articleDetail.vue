...
import { EMyBlogMainScrollEvent } from "@/eventBus"
...
export default Vue.extend({
  ...
  methods: {
    toAnchor(item: ArrayType<IBlogTranslation["toc"]>) {
      // console.log(item);
    ...
    setCurrentAnchor() {
      //todo
      //重构的时候要么用intersection observer,但是这东西应该是不行,毕竟咱们用的是v-html
      //我感觉我想用二分法，直接O(logn)
      let range = 50
      let res = ""
    ...
    async getComment() {
      ...
      await BlogApi.getComment({ blogId: this.$route.params.id })
        .then((v) => {
    ...
    getArticle() {
      ...
      BlogApi.getArticle(this.$route.params.id, i18n.locale)
        .then((v) => {
          this.article = v.data as IDetailArticle
          this.$nextTick(() => {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                this.anchorDomGettableFlag = true
              })
            })
          })
        })
  ...
  computed: {
    articleMenu(): IBlogTranslation["toc"] {
      const menu = this.translatedArticle?.toc ?? []
    ...
    anchorDoms(): HTMLElement[] {
      ...
      return getRealDoms((this.translatedArticle?.toc ?? []) || [])
    },
    ...
    translatedArticle() {
      ...
      return this.article?.translations.find((item) => item.lang === lang) || this.article?.translations.find((item) => item.lang === "zh")
    },
  },
})
</script>
  ...
  <div class="article-detail-container">
    <Layout>
      <div ref="main" class="main" v-loading="articleLoading">
        <ArticleContent :article="{ ...article, ...translatedArticle }" />
        <MessageArea @submit="handleSubmit" :list="commentsData.comments" :title="commentsData.title" :isListLoading="commentsData.loading" :subTitle="`(${commentsData.subTitle})`"></MessageArea>
      </div>
      <template #right>
