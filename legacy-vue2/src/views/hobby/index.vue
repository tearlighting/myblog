...
export default Vue.extend({
  ...
  methods: {
    getHobbyList() {
      ...
      HobbyApi.getHobbyList({ ...this.routeInfo })
        .then((v) => {
          if (v && v.data.total) {
  ...
  computed: {
    currentMusic(): IVueAplayerMusic {
      ...
      const res: IVueAplayerMusic = {
        ...
        title: (this.currentItem as any).title,
    ...
    <LayOut>
      <div class="hobby-content">
        <div class="content" ref="hobbyContent" v-loading="hobbyListLoading">
          <HobbyItem v-for="item of hobbyList" :content="item" :key="item.id" :class="{ current: currentItem?.id === item.id }" @categoryChange="blogCategoryChange" @playMusic="playMusic"></HobbyItem>
        </div>
      ...
      <template #right>
        <BlogMenu :list="categoryWithALL" @menuChange="blogCategoryChange" v-loading="hobbyMenuLoading"> </BlogMenu>
      </template>
    </LayOut>
  </div>
</template>
