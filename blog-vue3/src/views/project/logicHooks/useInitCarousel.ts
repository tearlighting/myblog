...
export const useInitCarousel = ({ target, track }: IUseInitCarouselProps) => {
    const { projectCarouselStore: { defineCarouselGlueIns }, projectSubPub: { subPubIns }, projectCardECarouselPhaseStateMachineRef } = useProjectStore()
    console.log(target);
    ...
    const gluer = new CarouselGlue(carouselAccumulatorIns, subPubIns).defineOnSwitch((dir) => {
