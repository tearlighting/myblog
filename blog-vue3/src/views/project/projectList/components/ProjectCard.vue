...
<template>
  <article role="project-card" class="size-full rounded-[16px] select-none">
    <div role="project-card__inner" class="size-full grid grid-cols-1 gap-[1.2rem] p-[1.6rem]">
      ...
      <div v-if="project.thumb" role="project-cover" class="w-full h-full rounded-[12px] overflow-hidden">
        <img :src="project.thumb" alt="" class="size-full object-cover pointer-events-none" />
      </div>
      ...
      <div role="project-body" class="flex flex-col gap-[0.6rem]">
        <header role="project-header" class="grid md:grid-cols-[1fr_auto] grid-cols-1">
          <h3 role="project-title" class="text-[1.2rem] font-semibold text-text tracking-wide overflow-hidden overflow-ellipsis whitespace-nowrap">
            {{ project.title }}
          </h3>
          <div role="project-detail" class="text-[0.88rem] text-muted leading-[1.7] max-w-[90%] flex items-center gap-[0.6rem]">
            ...
            <span class="flex items-center gap-[0.3rem]"> <SvgIcon :name="EIcons.Comment" class="text-xl"></SvgIcon>{{ project.commentNumber }} </span>
          </div>
        </header>
        ...
        <!-- CTA -->
        <a role="project-link" @click="clickDetail" class="mt-[0.8rem] text-[0.85rem] text-primary decoration-0 self-start hover:underline cursor-pointer"> View Project → </a>
      </div>
    </div>
  </article>
</template>
