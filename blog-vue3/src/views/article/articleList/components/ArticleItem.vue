  ...
  <div role="article-item-wrapper" class="size-full relative">
    <a role="row-inner" class="flex flex-col gap-[6px] rounded-[10px] no-underline">
      <div role="row-main">
        ...
        <p role="desc" class="text-[0.8rem] opacity-0.7 text-muted leading-1.5 mt-1">
          {{ item?.description }}
        </p>
      </div>
      ...
      <div role="row-meta" class="flex gap-[10px] text-[0.72rem] text-muted mt-[6px]">
        ...
        <span class="flex items-center" :style="{ opacity: heatOpacity(item.scanNumber) }"> <SvgIcon :name="EIcons.View" class="w-[12px] h-[12px] opacity-40 pr-1" />{{ item.scanNumber }} </span>
      </div>
    </a>
  </div>
</template>
