...
<template>
	<div role="article-wrapper" class="size-full grid grid-cols-[1fr] relative py-2">
		<div role="article-list-wrapper" class="size-full overflow-y-auto" :class="{ dimmed: switching }"
			ref="scrollerRef">
			<ul role="article-list" class="flex flex-col size-full text-text" ref="scrollerContentRef">
				<li v-for="item in translatedArticle" :key="item.id" role="article-row"
					class="relative px-[18px] w-full" :data-key="item.id">
					<ArticleItem :item="item" @clickTag="selectCategory" @click="toDetail(item.id)"></ArticleItem>
				</li>
				<!-- 最后加一个哨兵 -->
		...
		<div role="article-tags" class="absolute right-5 top-1/2 -translate-y-1/2">
			<ArticleTagRail :items="categories" :active-ids="activedCategories" @select="selectCategory">
			</ArticleTagRail>
		</div>
