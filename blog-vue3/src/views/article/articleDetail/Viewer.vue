...
import { ref } from "vue"
import { useInitScroller, useInitToc, useTranslatedArticle } from "./logicHooks"

const { translatedArticleItem } = useTranslatedArticle()
...
<template>
  <div role="article-detail" class="relative size-full grid lg:grid-cols-[1fr_200px] grid-cols-[1fr_0] overflow-y-auto" ref="scrollerRef">
    <main ref="scrollerContentRef" class="w-full min-w-0">
      <Article :article="translatedArticleItem?.htmlContent"></Article>
    </main>
    <nav role="article-toc" class="lg:w-full h-fit sticky lg:px-[8px] lg:py-[12px] text-sm text-muted w-fit p-[6px] lg:translate-x-0 -translate-x-[150%]" :style="{ top: TOC_OFFSET_TOP + 'px' }">
      <Toc v-if="deviceType === EDeviceType.DESKTOP" :items="translatedArticleItem?.toc" :to-anchor="toAnchor" :active-anchor="activeAnchor"></Toc>
      <template v-else>
        ...
        <n-drawer placement="right" :show="tocDrawerShow" :width="200" :on-update:show="(e) => (tocDrawerShow = e)">
          <Toc :items="translatedArticleItem?.toc" :to-anchor="toAnchor" :active-anchor="activeAnchor"></Toc>
        </n-drawer>
      </template>
    </nav>
  </div>
...
@import "@/components/Article/index.less";
[role="article-detail"] {
  .theme-scroller();
  .article-reading-skin();
  [role="article-toc"] {
