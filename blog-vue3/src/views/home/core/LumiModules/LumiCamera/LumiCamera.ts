...
export class LumiCamera {
  ...
  private readonly PRESSURE_RELEASE = 0.003 // 反向释放速度

  reset({ maxOffset = 0, offset = maxOffset / 2, pressure = 0 }: { maxOffset: number; offset?: number; pressure?: number }) {
  ...
  consume(deltaY: number): LumiCameraState {
    ...
    const atBoundarySameDir = boundary === "bottom" ? dir > 0 : boundary === "top" ? dir < 0 : false
