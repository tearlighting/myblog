...
const {
	lumiStore: { scene, initailLumiEffect, camera, lumiRender, lumiText },
} = useHomeStore()
...
watch(activeIndex, async () => {
	...
	if (imgRef.value) {
		lumiRender.switchImg(imgRef.value)
	}
			...
			<div role="lumi-card" class="relative size-full overflow-hidden z-1" ref="cardRef">
				<div role="lumi-glass" class=" size-full" ref="glassRef">
					<div role="lumi-blur" class=" size-full" ref="blurRef">
						<!-- 做translate -->
						<div role="lumi-media" class=" xl:h-auto h-full relative w-full min-h-full opacity-0"
							ref="mediaRef">
							<img :key="props.slides[activeIndex].bigImg" :src="props.slides[activeIndex].bigImg"
						...
						<div role="lumi-text"
							class="xl:max-w-[420px]	 xl:left-[8%] xl:bottom-[18%] left-[6%] bottom-[14%] max-w-[90%] absolute  z-5  opacity-100"
							ref="textRef">
							<h2 role="title" class="mb-[10px] font-medium w-fit" ref="titleRef">{{
								props.slides[activeIndex].title }}</h2>
							<p role="description" ref="descriptionRef" class="font-normal">{{
								props.slides[activeIndex].description }}</p>
						</div>
					</div>
				</div>
			</div>
