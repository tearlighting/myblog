...
<template>
	<div class="h-full transition-all duration-300 select-none" role="side-menu"
		...
		<div v-if="deviceType === EDeviceType.DESKTOP" class="flex flex-col">
			<AppLogo />
			<!-- 桌面菜单 -->
			<n-menu :options="menuOptions" :value="activeKey" :expanded-keys="expandedKeys" :collapsed="isCollapse"
				@update:value="onSelect" @update:expanded-keys="onExpanded"
		...
		<n-drawer v-else :show="!isHidden" width="240" placement="left" :on-update:show="toggleMenu">
			<div class="flex flex-col">
				<AppLogo />
				<n-menu :options="menuOptions" :value="activeKey" :expanded-keys="expandedKeys" @update:value="onSelect"
					@update:expanded-keys="onExpanded" class="flex-1 bg-bg w-full" />
			</div>
		</n-drawer>
	</div>
</template>
